...
// std::atomic<int> currentRow(0); 
 // use mutexes and volatile variables instead of atomic
volatile int y =0;
...
void calculateMandelbrotTask(void *parameter) {
  while(1) {
    ...
    xSemaphoreTake(y_blocker, portMAX_DELAY);
    y++;
    xSemaphoreGive(y_blocker);
...
void setup() {
  ...
  //create a mutex, remove later if using atomic or half
  y_blocker=xSemaphoreCreateMutex();
